import { neon } from "@neondatabase/serverless";
import { config } from "dotenv";

config({ path: ".env.local", quiet: true });

const databaseUrl = process.env.DATABASE_URL_UNPOOLED ?? process.env.DATABASE_URL;

if (!databaseUrl) {
  throw new Error("DATABASE_URL_UNPOOLED or DATABASE_URL is required.");
}

const expectedTables = [
  "assets",
  "auth_identities",
  "auth_sessions",
  "auth_users",
  "birth_profiles",
  "chart_aspects",
  "chart_calculations",
  "chart_findings",
  "chart_houses",
  "chart_placements",
  "clients",
  "compatibility_reports",
  "consent_records",
  "dasha_periods",
  "generated_artifacts",
];

/**
 * Tables a migration has removed. Present in the live schema means a migration
 * did not run, or ran against a different branch than the one being checked.
 *
 * `auth_credentials` joins the list in 0005: email/password sign-in was never
 * built, so the table never held a row.
 *
 * `workspaces` and `workspace_members` join it in 0006. They held the tenant
 * boundary that let a signed-out browser own data as `anon:<device>`; Google
 * became the only way in, so `auth_users.id` is the tenant key now and the
 * join table was a rename of `auth_users` with extra steps. Unlike
 * `auth_credentials`, these two did hold rows — 0006 backfills `user_id` from
 * them before dropping them, so finding them still present means that
 * migration has not run here.
 */
const deferredTables = [
  "audit_events",
  "auth_credentials",
  "client_notes",
  "client_tags",
  "consultations",
  "tags",
  "workspace_members",
  "workspaces",
];

const sql = neon(databaseUrl);
const liveTables = await sql.query(
  "select table_name from information_schema.tables where table_schema = 'public' and table_type = 'BASE TABLE' order by table_name",
);
const liveTableNames = liveTables.map(({ table_name: tableName }) => tableName);
const missingTables = expectedTables.filter((tableName) => !liveTableNames.includes(tableName));
const unexpectedDeferredTables = deferredTables.filter((tableName) =>
  liveTableNames.includes(tableName),
);

if (missingTables.length > 0) {
  throw new Error(`Missing database tables: ${missingTables.join(", ")}`);
}

if (unexpectedDeferredTables.length > 0) {
  throw new Error(`Deferred tables still exist: ${unexpectedDeferredTables.join(", ")}`);
}

await sql.query(`
DO $schema_verification$
DECLARE
  user_one uuid := gen_random_uuid();
  user_two uuid := gen_random_uuid();
  client_one uuid := gen_random_uuid();
  client_partner uuid := gen_random_uuid();
  client_other_user uuid := gen_random_uuid();
  birth_one uuid := gen_random_uuid();
  birth_partner uuid := gen_random_uuid();
  birth_other_user uuid := gen_random_uuid();
  chart_one uuid := gen_random_uuid();
  chart_partner uuid := gen_random_uuid();
  chart_other_user uuid := gen_random_uuid();
  asset_one uuid := gen_random_uuid();
  artifact_one uuid := gen_random_uuid();
  consent_one uuid := gen_random_uuid();
  consent_partner uuid := gen_random_uuid();
  consent_other_user uuid := gen_random_uuid();
  invalid_phone_rejected boolean := false;
  cross_user_link_rejected boolean := false;
  missing_consent_rejected boolean := false;
BEGIN
  INSERT INTO auth_users (id, email, display_name) VALUES
    (user_one, 'codex-schema-verification@example.invalid', 'Codex schema verification'),
    (user_two, 'codex-tenant-verification@example.invalid', 'Codex tenant boundary verification');

  INSERT INTO clients (
    id, user_id, display_name, email, phone_e164,
    preferred_contact_method, updated_at
  ) VALUES
    (client_one, user_one, 'Verification Client', 'verify@example.invalid',
      '+15555550123', 'email', timestamp with time zone '2000-01-01 00:00:00+00'),
    (client_partner, user_one, 'Verification Partner', null, null, 'none', now()),
    (client_other_user, user_two, 'Other Account Client', null, null, 'none', now());

  UPDATE clients SET display_name = 'Verification Client Updated' WHERE id = client_one;
  IF (SELECT updated_at <= timestamp with time zone '2000-01-01 00:00:00+00' FROM clients WHERE id = client_one) THEN
    RAISE EXCEPTION 'updated_at trigger did not run';
  END IF;

  BEGIN
    INSERT INTO clients (user_id, display_name, phone_e164)
    VALUES (user_one, 'Invalid Phone', '555-1234');
  EXCEPTION WHEN check_violation THEN
    invalid_phone_rejected := true;
  END;
  IF NOT invalid_phone_rejected THEN
    RAISE EXCEPTION 'phone constraint did not reject invalid data';
  END IF;

  INSERT INTO consent_records (
    id, user_id, client_id, purpose, policy_version, granted_at, capture_source
  ) VALUES
    (consent_one, user_one, client_one, 'store_birth_details', 'v0-verification',
      now(), 'schema_verification'),
    (consent_partner, user_one, client_partner, 'store_birth_details', 'v0-verification',
      now(), 'schema_verification'),
    (consent_other_user, user_two, client_other_user, 'store_birth_details',
      'v0-verification', now(), 'schema_verification');

  INSERT INTO birth_profiles (
    id, user_id, client_id, consent_record_id, birth_date, reported_birth_time,
    calculation_birth_time, birth_time_accuracy, calculation_time_is_fallback,
    latitude, longitude, time_zone_id, resolved_utc_offset_minutes,
    timezone_source, birth_instant_utc
  ) VALUES
    (birth_one, user_one, client_one, consent_one, date '1990-06-15', null, time '12:00:00',
      'unknown', true, 28.613900, 77.209000, 'Asia/Kolkata', 330,
      'coordinates', timestamp with time zone '1990-06-15 06:30:00+00'),
    (birth_partner, user_one, client_partner, consent_partner, date '1992-02-20',
      time '08:30:00',
      time '08:30:00', 'exact', false, 40.712800, -74.006000, 'America/New_York', -300,
      'coordinates', timestamp with time zone '1992-02-20 13:30:00+00'),
    (birth_other_user, user_two, client_other_user, consent_other_user,
      date '1988-10-10', null,
      time '12:00:00', 'unknown', true, 51.507400, -0.127800, 'Europe/London', 60,
      'coordinates', timestamp with time zone '1988-10-10 11:00:00+00');

  -- The whole point of the consent column: birth facts with no recorded
  -- permission must not be storable, however the caller reaches the table.
  BEGIN
    INSERT INTO birth_profiles (
      user_id, client_id, consent_record_id, birth_date,
      calculation_birth_time, calculation_time_is_fallback
    ) VALUES (user_one, client_one, null, date '1990-06-15', time '12:00:00', true);
  EXCEPTION WHEN not_null_violation THEN
    missing_consent_rejected := true;
  END;
  IF NOT missing_consent_rejected THEN
    RAISE EXCEPTION 'birth profile without a consent record was accepted';
  END IF;

  INSERT INTO chart_calculations (
    id, user_id, birth_profile_id, input_fingerprint, input_snapshot_json,
    engine_id, calculation_version, ascendant_sign, sun_sign, moon_sign
  ) VALUES
    (chart_one, user_one, birth_one, repeat('a', 64), '{"source":"verification"}'::jsonb,
      'verification-engine', '1', 'Aries', 'Gemini', 'Cancer'),
    (chart_partner, user_one, birth_partner, repeat('b', 64), '{"source":"verification"}'::jsonb,
      'verification-engine', '1', 'Taurus', 'Pisces', 'Leo'),
    (chart_other_user, user_two, birth_other_user, repeat('c', 64),
      '{"source":"verification"}'::jsonb, 'verification-engine', '1',
      'Virgo', 'Libra', 'Scorpio');

  INSERT INTO chart_placements (
    user_id, chart_id, point_code, longitude, sign_code, degree_in_sign, house_number
  ) VALUES (user_one, chart_one, 'Sun', 75.25, 'Gemini', 15.25, 3);

  INSERT INTO chart_houses (user_id, chart_id, house_number, sign_code, cusp_longitude)
  VALUES (user_one, chart_one, 1, 'Aries', 10.5);

  INSERT INTO chart_aspects (
    user_id, chart_id, from_point, to_point, aspect_type, exact_angle, orb
  ) VALUES (user_one, chart_one, 'Sun', 'Moon', 'square', 90, 1.25);

  INSERT INTO dasha_periods (user_id, chart_id, level, lord, start_at, end_at)
  VALUES (user_one, chart_one, 1, 'Venus', now(), now() + interval '1 year');

  INSERT INTO chart_findings (
    user_id, chart_id, instance_key, rule_id, category, tier,
    priority, strength, score, selected, rank
  ) VALUES (
    user_one, chart_one, 'verification.finding', 'verification.rule',
    'core', 'foundation', 'high', 0.8, 0.7, true, 1
  );

  INSERT INTO compatibility_reports (
    user_id, primary_client_id, partner_client_id, primary_chart_id,
    partner_chart_id, compatibility_score, algorithm_version, input_fingerprint
  ) VALUES (
    user_one, client_one, client_partner, chart_one, chart_partner,
    82.50, 'verification-1', repeat('d', 64)
  );

  INSERT INTO assets (
    id, user_id, client_id, storage_provider, object_key,
    mime_type, byte_size, sha256
  ) VALUES (
    asset_one, user_one, client_one, 'verification',
    'schema-verification/object', 'application/pdf', 100, repeat('e', 64)
  );

  INSERT INTO generated_artifacts (
    id, user_id, client_id, chart_id, source_asset_id,
    output_asset_id, artifact_type, generator_version
  ) VALUES (
    artifact_one, user_one, client_one, chart_one, asset_one,
    asset_one, 'verification_report', '1'
  );

  BEGIN
    UPDATE generated_artifacts SET chart_id = chart_other_user WHERE id = artifact_one;
  EXCEPTION WHEN foreign_key_violation THEN
    cross_user_link_rejected := true;
  END;
  IF NOT cross_user_link_rejected THEN
    RAISE EXCEPTION 'cross-account optional link was not rejected';
  END IF;

  DELETE FROM chart_calculations WHERE id = chart_one;
  IF (SELECT chart_id IS NOT NULL FROM generated_artifacts WHERE id = artifact_one) THEN
    RAISE EXCEPTION 'chart delete did not clear artifact link';
  END IF;

  DELETE FROM assets WHERE id = asset_one;
  IF (SELECT source_asset_id IS NOT NULL OR output_asset_id IS NOT NULL FROM generated_artifacts WHERE id = artifact_one) THEN
    RAISE EXCEPTION 'asset delete did not clear artifact links';
  END IF;

  DELETE FROM auth_users WHERE id IN (user_one, user_two);
END;
$schema_verification$;
`);

const [{ remaining_test_rows: remainingTestRows }] = await sql.query(
  "select count(*)::int as remaining_test_rows from auth_users where email like 'codex-%-verification@example.invalid'",
);

if (remainingTestRows !== 0) {
  throw new Error("Database verification left temporary rows behind.");
}

const [{ migration_count: migrationCount }] = await sql.query(
  "select count(*)::int as migration_count from drizzle.__drizzle_migrations",
);

console.log(
  JSON.stringify({
    database: "connected",
    tables: liveTableNames.length,
    migrations: migrationCount,
    smokeTest: "passed",
    temporaryRowsRemaining: remainingTestRows,
  }),
);
